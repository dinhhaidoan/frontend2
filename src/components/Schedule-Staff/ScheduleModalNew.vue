            ...
            <div class="form-group full-width">
              ...
              <div class="schedule-builder">
                <div class="day-time-selector">
                  ...
                  <!-- Time Slots Grid -->
                  <div v-if="currentDay" class="time-slots-grid">
                    <label 
                      v-for="slot in timeSlotOptions" 
                      :key="slot.value"
                      class="time-slot-checkbox"
                      :class="{
                        'selected': currentTimeSlots.includes(slot.value),
                  ...
                  <!-- Add Day Button -->
                  <div v-if="currentDay && currentTimeSlots.length > 0" class="add-day-section">
                    <div class="selected-preview">
                      <strong>Thứ {{ currentDay }}:</strong> {{ getCurrentSlotsText() }}
                      <span class="duration">({{ getCurrentDuration() }} phút)</span>
                    </div>
                    <button 
                      type="button" 
                  ...
                  >
                    <div class="day-info">
                      <div class="day-header">
                        <strong>Thứ {{ daySchedule.day }}</strong>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // Get current slots text
    const getCurrentSlotsText = () => {
      if (currentTimeSlots.value.length === 0) return ''
      
      const slots = currentTimeSlots.value
    ...
    const getCurrentDuration = () => {
      return currentTimeSlots.value.length * 45 // 45 minutes per slot
    }
    ...
    const addScheduleDay = () => {
      if (currentDay.value && currentTimeSlots.value.length > 0) {
        // Remove existing schedule for this day
        scheduleDays.value = scheduleDays.value.filter(day => day.day !== currentDay.value)
        
        // Add new schedule
        scheduleDays.value.push({
          day: currentDay.value,
          slots: [...currentTimeSlots.value],
          slotsText: getCurrentSlotsText(),
