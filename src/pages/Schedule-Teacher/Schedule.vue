<template>
  <div class="teacher-schedule-container">
    ...
    <ScheduleDetailModal v-if="showScheduleModal" :schedule="selectedSchedule" @close="closeScheduleModal" @action="handleScheduleAction" />
    <ClassDetailModal v-if="showClassModal" :class-data="selectedClass" @close="closeClassModal" @action="handleClassAction" />
    <AttendanceModal v-if="showAttendanceModal" :schedule="attendanceSchedule" @close="closeAttendanceModal" @save="saveAttendance" />
  </div>
</template>
<script setup>
import { ref, computed, onMounted } from 'vue'
import { useRouter } from 'vue-router'
import QuickStats from '@/components/Schedule-Teacher/QuickStats.vue'
import MiniCalendar from '@/components/Schedule-Teacher/MiniCalendar.vue'
import ScheduleFilters from '@/components/Schedule-Teacher/ScheduleFilters.vue'
import DailyView from '@/components/Schedule-Teacher/DailyView.vue'
import WeeklyView from '@/components/Schedule-Teacher/WeeklyView.vue'
import MonthlyView from '@/components/Schedule-Teacher/MonthlyView.vue'
import ScheduleDetailModal from '@/components/Schedule-Teacher/ScheduleDetailModal.vue'
import ClassDetailModal from '@/components/Schedule-Teacher/ClassDetailModal.vue'
import AttendanceModal from '@/components/Schedule-Teacher/AttendanceModal.vue'
...
const stats = ref({ todayClasses: 4, weekClasses: 18, totalStudents: 245, upcomingExams: 3, pendingGrades: 12, attendance: 95 })
const views = [
...
const handleScheduleAction = (action, data) => { 
  if (action === 'attendance') {
    attendanceSchedule.value = selectedSchedule.value
    showAttendanceModal.value = true
    closeScheduleModal()
  } else {
    console.log('Schedule action:', action, data)
    closeScheduleModal()
  }
}
const handleClassAction = (action, data) => { 
  if (action === 'attendance') {
    attendanceSchedule.value = data
    showAttendanceModal.value = true
    closeClassModal()
  } else {
    console.log('Class action:', action, data)
    closeClassModal()
  }
}
...
onMounted(() => { console.log('Teacher Schedule Page loaded') })
</script>
<style scoped>
.teacher-schedule-container { min-height: 100vh; background: #f5f7fa; padding: 20px; }
...
@media (max-width: 768px) {
  .teacher-schedule-container { padding: 12px; }
